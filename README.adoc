...
Bemerkung: Die Variablen sind frei zu wählen (angenommen es ist noch keine DB vorhanden), jedoch muss gelten:

* POSTGRES_USER und DB_USER müssen gleich sein
* POSTGRES_PASSWORD und DB_PW müssen gleich sein
* CLIENT_ID und CLIENT_SECRET müssen mit GitHub-OAuth-Apps generiert werden 

Folgende Schritte sind zum Starten der Anwendung notwendig:

1. Anlegen einer .env Datei unter /Splitter mit dem Inhalt:
```
POSTGRES_DB=jdbc
POSTGRES_USER=
POSTGRES_PASSWORD=
...
2. Start der Anwendung mit "docker compose up" (Wenn der source code verändert wurde mit "docker compose up --build" starten)
...
Außerdem werden Sie von unseren Tutor:innen regelmäßig besucht und haben dort ebenfalls die Gelegenheit, Fragen zu stellen. Kristin, Leonie, oño und Jens kommen Sie auch gelegentlich besuchen oder helfen aus, wenn besonders schwierige Fragen auftreten sollten. 
